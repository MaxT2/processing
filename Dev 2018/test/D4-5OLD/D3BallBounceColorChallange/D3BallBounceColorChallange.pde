//After completeing the ball bounce program with the kids
// challange them to make the ball change colors every time it hits a wall
// they will need a color variable...
color ballColor = color(0,255,0);

// then they will need to set the fill before the draw the ball... see draw() method below
...
void draw(){
  background(255);
  // <---------> set the color of the ball to the color variable
  fill(ballColor);  
  // then you need to change the color of the ballColor
  // variable every time you hit a wall or reverse the speed
  // using ballSpeedX = ballSpeedX * -1; or ballSpeedY = ballSpeedY * -1;
  // finished code is below in each if statment marked with <------>
...
// control ball with arrow keys
  ...
  if(keyPressed && key == CODED && keyCode == DOWN){
   ...
   if(ballX > width - ballRadius){
    ...
    // <------------------>
    ballColor = color(0,0,255); // change color to blue
   }
   ...
   if(ballX < 0 + ballRadius){
    ...
    ballSpeedX = ballSpeedX * -1;
    // <-------------->
        ballColor = color(255,0,0); // change color to red
   }
   ...
   if(ballY > height - ballRadius){
    ...
    // <----------------->
        ballColor = color(255,255,0); // change color to yellow
   }
   ...
   // <---------> randomly change color
       ballColor = color(random(0-255),random(0-255),random(0-255)); // change color to blue
   }
